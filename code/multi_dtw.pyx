...
def dtw_grid(double [:,:] var_1, double [:,:] var_2,long lim_r, double pen_r):

    #Define variables used for looping
    cdef long var_1_x = var_1.shape[0]   
    cdef long var_1_y = var_1.shape[1]   
    cdef long var_2_x = var_2.shape[0]   
    cdef long var_2_y = var_2.shape[1]   
    cdef long x1,y1,y2
    cdef double [:,:,:] diff_arr = np.zeros((var_1_x,var_1_y,var_2_y),dtype=DTYPE)
    cdef double m,dy,penalty
...
def dtw_single(double [:] var_1, double [:] var_2,long lim_1, long lim_2, double pen_r):
    ...
    for x1 in range(var_1_x):
        ...
        for x2 in range(var_2_x):
            ...
            #store the difference
            diff_arr[x1,x2] = abs(var_1[x1]-var_2[x2])+cumulative#**2+penalty#*d_x/lim_r
...
def dtw_path(double [:,:] var_1, double [:,:] var_2,long lim_r, double pen_r, long cost_matrix):

    cdef long var_1_x = var_1.shape[0]   
    cdef long var_1_y = var_1.shape[1]   
    cdef long var_2_x = var_2.shape[0]   
    cdef long var_2_y = var_2.shape[1]   
    cdef long i=var_1_y-1,j=var_2_y-1,l=0
    cdef long max_len = max([var_1_y,var_2_y])
    cdef long min_len = max([var_1_y,var_2_y])
    cdef double rat_len = float(max_len)/float(min_len)
    cdef double[:,:,:] diff_arr = np.zeros((var_1_x,var_1_y,var_2_y),dtype=DTYPE)
    cdef long [:] path_var_1 = np.zeros(max_len*int(rat_len+1.),dtype=uTYPE)-999
    cdef long [:] path_var_2 = np.zeros(max_len*int(rat_len+1.),dtype=uTYPE)-999
    cdef double i_cost,j_cost,b_cost
    ...
    #flip the order of the output array
    path_var_1 = np.array(path_var_1)[::-1]
    path_var_2 = np.array(path_var_2)[::-1]

    #remove all -999 values in return
    if cost_matrix == 1:
        return np.array(path_var_1[l+3:]),np.array(path_var_2[l+3:]),np.array(diff_arr)
    else:                         
        return np.array(path_var_1[l+3:]),np.array(path_var_2[l+3:])